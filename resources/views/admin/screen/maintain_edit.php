            ...
            <form action="{{ $url_action }}" method="post" accept-charset="UTF-8" class="form-horizontal" id="form-main"
                ...
                <div class="box-body">
                    <div class="fields-group">
                        ...
                        <div
                            class="form-group {{ $errors->has('descriptions.'.$code.'.maintain') ? ' has-error' : '' }}">
                            <label for="{{ $code }}__maintain"
                                class="col-sm-2 control-label">{{ trans('news.maintain') }}</label>
                            <div class="col-sm-8">
                                <textarea id="{{ $code }}__maintain" class="editor"
                                    name="descriptions[{{ $code }}][maintain]">
                                        {{ old('descriptions.'.$code.'.maintain',($descriptions[$code]['maintain']??'')) }}
                                    </textarea>
                                @if ($errors->has('descriptions.'.$code.'.maintain'))
                                <span class="help-block">
                                    <i class="fa fa-info-circle"></i> {{ $errors->first('descriptions.'.$code.'.maintain') }}
                                </span>
                                @endif
                            </div>
                        </div>
